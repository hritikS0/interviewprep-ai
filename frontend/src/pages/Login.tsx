...
export default function Login() {
  ...
  // Auto redirect if already authenticated or pending confirmation
  useEffect(() => {
    if (localStorage.getItem('isAuthenticated') === 'true') {
      navigate('/dashboard', { replace: true })
    } else if (localStorage.getItem('pendingEmailConfirmation') === 'true') {
      ...
      <div className="hidden lg:flex lg:w-1/2 bg-zinc-950 text-white relative flex-col justify-between p-12 overflow-hidden">
        ...
        <div className="max-w-[440px] space-y-8 relative z-10">
          <h2 className="text-[36px] font-extrabold tracking-tight leading-tight">
            Accelerate your prep with adaptive AI mock loops.
          </h2>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="space-y-1.5">
              <div className="flex items-center justify-between">
                ...
                <button
                  type="button"
                  onClick={() => alert('Forgot password simulation: Check console for mock recovery token.')}
                  className="text-[11px] font-bold text-accent hover:text-accent-hover transition-colors focus:outline-none"
                >
                  Forgot password?
                </button>
              </div>
